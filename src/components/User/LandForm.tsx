...
const LandForm = () => {
	...
	const [landData, setLandData] = useState<Record<string, string>>({
		serial: "494437282472",
		challan: "",
		officeName: "",
		mouza: "",
	...
	const handleSubmit = async (e: any) => {
		...
		if (
			...
			toast.error(
				"You don't have sufficient balance! Please recharge your account."
			);
			return;
		}

		try {
			const response = await baseApi("/land/create", {
				method: "POST",
				body: {
					landData,
					collectionData,
			...
			router.push(`/user/land/print/${response?.data?._id}`);
		} catch (error: any) {
			console.error(error);
	...
	return (
		<div className="md:p-6 p-3  bg-white  ">
			...
			<form onSubmit={handleSubmit} className="space-y-6">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
					<div>
						<label className="">ক্রমিক নং:</label>
						<input
							name="serial"
				...
				<div className="flex flex-wrap md:gap-4 gap-3">
					{/* পরিশোধের সাল (BN) */}
					<div className="md:flex-1 md:max-w-[400px] w-full ">
				...
				<div>
					...
					<table className="w-full   border border-gray-300    border-collapse mt-2">
						<tbody className="w-full">
							{owners.map((owner, i) => (
								<tr key={i}>
									<td className="md:px-4 w-[30px] py-2 text-center">{i + 1}</td>
									...
									<td className="md:px-4 px-1 py-3">
										<input
											...
											className="border border-gray-300 rounded-md md:px-3 px-2 py-2 w-full"
										/>
									</td>
								...
								<tr>
									...
									<td className="px-4 py-2">
										<input
											required
											name="remarks"
											value={collectionData.remarks || 0}
											onChange={e =>
												setCollectionData({
													...collectionData,
													[e.target.name]: e.target.value,
